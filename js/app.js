...
// Enemies our player avoid
var Enemy = function (x, y, sprite) {
    // Variables applied to each of our instances go here, we've provided one for
    // you to get started
    this.x = x;
    this.y = y;
    // The image/sprite for our enemies, this uses a helper we've provided to easily
    // load images
...
Enemy.prototype.update = function (dt) {
    ...
    // Check collisions between enemy & player
    function checkCollision(enemy, player) {
        return (player.x > enemy.x - enemy.hitBox.x / 2 && player.x < enemy.x + enemy.hitBox.x / 2 && player.y > enemy.y - enemy.hitBox.y / 2 && player.y < enemy.y + enemy.hitBox.y / 2);
    }
...
Player.prototype.update = function () {
    ...
    if (this.y < 0) {
        alert("You woooooooonnnnnnnnnnnn");
        this.reset();
    }
    this.action = null;
};
